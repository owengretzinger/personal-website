...
import TextLink, { ResumeLink, TextLinkPassProps } from '@/components/textLink';
...
import CoverLetterContent from '@/components/coverLetter';
...
function ResumeProject({ title, href, projectType, description }: { title: string, href: string, projectType: string, description: ReactNode }) {
  return (
    <div>
      <ResumeLink text={title} href={href} icon="link" target="_blank" isPublic={showColours} />
...
function CoverLetter() {
  ...
  // text.dangerousSetInnerHTML()

  return (
    <>
      <Header />
      <div className="w-full h-full mt-10 whitespace-pre-line">
        <CoverLetterContent />
      </div>
    </>
  )
