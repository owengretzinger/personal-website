...
export default function Nav({ showOnLargeScreens = true, ...props }) {
  ...
  return (
    <>
      {/* nav bar */}
      <nav className={`fixed top-0 w-full px-10 py-2 z-30 bg-white transition duration-500 ${scrollDirection === "up" ? "translate-y-0" : "-translate-y-20"} ${showOnLargeScreens ? "block" : "lg:hidden"}`}>
      ...
      {/* mobile drawer */}
      <div className={`top-0 lg:hidden fixed z-50 w-screen h-screen ${mobileNavOpen ? "backdrop-blur-sm pointer-events-auto" : "backdrop-blur-none pointer-events-none"} transition-all duration-500`}>
